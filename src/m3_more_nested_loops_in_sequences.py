...
def run_test_largest_number():
    ...
    #Test 4:
    expected = 120
    answer = largest_number(([3,5,4,6,7,8], [2,5,6,7,2,90,-10], [20,40,60,80,120]))
    print('Expected and actual are:', expected, answer)
...
def first_is_elsewhere_too(seq_seq):
    ...
    for k in range(len(sequence)):
        sequence1=sequence[k]
        for j in range(len(seq_seq)-1):
            othersequence=seq_seq[j+1]
            for n in range(len(othersequence)):
                if sequence1==othersequence[n]:
                    return True
    return False
